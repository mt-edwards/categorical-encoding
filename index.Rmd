...
The hash encoding method uses a `hash_method` to transform $N_c$ categorical values to $N_n=2^\texttt{n_components}$ numerical (integer) values. Note that $N_n$ is often much smaller than $N_c$. The hash encoding method then one-hot encodes the integer values.
